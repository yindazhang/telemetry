...
void build_fat_tree_routing(
	...
	if((OrbWeaver == 0x2) || (OrbWeaver & 0x3) == 0x3){
		if(storeConfig){
			...
			for(uint32_t aggId = 1;aggId <= K;++aggId){
				edges[0]->SetDeviceGenerateGap(K * RATIO + aggId, 5 * base);
				edges[0]->AddTeleRouteTo(0, K * RATIO + aggId);
			}
			for(uint32_t i = 1;i < K * RATIO - 1;++i){
				for(uint32_t aggId = 1;aggId <= K;++aggId){
					edges[i]->SetDeviceGenerateGap(K * RATIO + aggId, 5 * base);
					edges[i]->AddTeleRouteTo(0, K * RATIO + aggId);
					edges[i]->AddTeleRouteTo(1, K * RATIO + aggId);
				}
			}
			for(uint32_t aggId = 1;aggId <= K;++aggId){
				edges[K*RATIO-1]->SetDeviceGenerateGap(K * RATIO + aggId, 5 * base);
				edges[K*RATIO-1]->AddTeleRouteTo(1, K * RATIO + aggId);
			}

			for(uint32_t j = 0;j < K;++j){
				for(uint32_t coreId = 1;coreId <= K;++coreId){
					aggregations[j]->SetDeviceGenerateGap(K + coreId, 5 * base);
			...
			for(uint32_t i = 1;i < NUM_BLOCK - 1;++i){
				for(uint32_t j = 0;j < K;++j){
					for(uint32_t coreId = 1;coreId <= K;++coreId){
						aggregations[i*K+j]->SetDeviceGenerateGap(K + coreId, 5 * base);
			...
			for(uint32_t j = 0;j < K;++j){
				for(uint32_t coreId = 1;coreId <= K;++coreId){
					aggregations[(NUM_BLOCK-1)*K+j]->SetDeviceGenerateGap(K + coreId, 5 * base);
			...
			for(uint32_t i = 0;i < K * K;++i){
				cores[i]->SetDeviceGenerateGap(NUM_BLOCK, 5 * base);
				cores[i]->AddTeleRouteTo(0, NUM_BLOCK);
				cores[i]->SetDeviceGenerateGap(1, 5 * base);
		...
		if(storeConfig){
			...
			for(uint32_t j = 0;j < K;++j){
				for(uint32_t k = 1; k <= 2*K;++k){
					aggregations[j]->SetDeviceGenerateGap(k, 5 * base);
					if(k != 1)
			...
			for(uint32_t i = 1;i < NUM_BLOCK - 1;++i){
				for(uint32_t j = 0;j < K;++j){
					for(uint32_t edgeId = 1;edgeId <= K;++edgeId){
						aggregations[i*K+j]->SetDeviceGenerateGap(edgeId, 5 * base);
			...
			for(uint32_t j = 0;j < K;++j){
				for(uint32_t k = 1; k <= 2*K;++k){
					aggregations[(NUM_BLOCK-1)*K+j]->SetDeviceGenerateGap(k, 5 * base);
					if(k != K)
			...
			for(uint32_t i = 0;i < K * K;++i){
				for(uint32_t j = 1;j <= NUM_BLOCK;++j){
					cores[i]->SetDeviceGenerateGap(j, 5 * base);
					if(j != NUM_BLOCK)
