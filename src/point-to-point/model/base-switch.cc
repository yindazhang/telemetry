...
TypeId
BaseSwitch::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::BaseSwitch")
            .SetParent<Node>()
            .SetGroupName("PointToPoint")
            .AddConstructor<BaseSwitch>();
    return tid;
}

BaseSwitch::BaseSwitch() : Node() {
    m_orbweaver = false;
    m_table.resize(arrSize);
    Simulator::Schedule(Seconds(2), &BaseSwitch::GeneratePacket, this);
}

void
BaseSwitch::SetOrbWeaver(uint32_t OrbWeaver){
    m_orbweaver = ((OrbWeaver & 0x1) == 0x1);
}

void 
BaseSwitch::SetEcmp(uint32_t Ecmp){
    m_ecmp = Ecmp;
}

void
BaseSwitch::AddHostRouteTo(Ipv4Address dest, uint32_t devId)
{
    m_routeForward[dest.Get()].push_back(devId);
}

void 
BaseSwitch::SetGenerateGap(uint32_t devId, uint32_t generateGap){
    DeviceProperty tmp;
    tmp.devId = devId;
...
void
BaseSwitch::SetDeviceCollector(uint32_t devId)
{
...
uint32_t
BaseSwitch::AddDevice(Ptr<NetDevice> device)
{
    ...
    device->SetReceiveCallback(MakeCallback(&BaseSwitch::ReceiveFromDevice, this));
...
bool
BaseSwitch::AddPathHeader(Ptr<Packet> packet)
{   
...
Ptr<Packet> 
BaseSwitch::CreatePacket(uint8_t type)
{
...
BaseSwitch::GeneratePacket(){
    if(m_orbweaver){
        ...
        Simulator::Schedule(NanoSeconds(nextTime - nsNow), &BaseSwitch::GeneratePacket, this);
    }
}

bool
BaseSwitch::IngressPipelineUser(Ptr<Packet> packet)
{
    // Start Parse Header
    Ipv4Header ipHeader;
    TcpHeader tcpHeader;
...
bool 
BaseSwitch::EgressPipelineUser(Ptr<Packet> packet){
    if(!m_orbweaver)
        return true;
...
bool
BaseSwitch::IngressPipelineIdle(Ptr<Packet> packet, Ptr<NetDevice> dev)
{
...
bool 
BaseSwitch::EgressPipelineIdle(Ptr<Packet> packet, Ptr<NetDevice> dev){
    TelemetryHeader teleHeader;
    packet->RemoveHeader(teleHeader);
...
bool
BaseSwitch::EgressPipeline(Ptr<Packet> packet, uint32_t priority, uint16_t protocol, Ptr<NetDevice> dev){
    if(priority == 0){
        if(protocol != 0x0800){
...
bool
BaseSwitch::IngressPipeline(Ptr<Packet> packet, uint32_t priority, uint16_t protocol, Ptr<NetDevice> dev){
    if(priority == 0){
        if(protocol != 0x0800){
            std::cout << "Unknown protocol for User packet" << std::endl;
            return false;
...
bool
BaseSwitch::ReceiveFromDevice(Ptr<NetDevice> device,
                                  Ptr<const Packet> p,
                                  uint16_t protocol,
                                  const Address& from)
{
