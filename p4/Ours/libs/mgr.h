...
static void parse_options(bf_switchd_context_t *sd_ctx, orbweaver_config_t *ow_config,
                          int argc,char **argv) {
  int option_index = 0;
  enum opts {
    OPT_INSTALLDIR = 1,
  ...
  while (1) {
    int c = getopt_long(argc, argv, "ht:p:g:n", options, &option_index);
    ...
    switch (c) {
      case 't':
        ow_config->ethertype_seed = (uint16_t)strtol(optarg, NULL, 0);
	break;
      case 'p':
        ow_config->prot_seed = (uint8_t)strtol(optarg, NULL, 0);
	break;
      case 'g':
        ow_config->gap_seed = atoi(optarg);
	break;
      case 'n':
