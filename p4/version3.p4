...
#define BUFFER_UPPER 768
#define BUFFER_LOWER 256
...
control Ingress(
    ...
    apply {
        ...
        else if(md.ow_md.type_ == ETHERTYPE_PULL) {
            ig_tm_md.ucast_egress_port = ig_intr_md.ingress_port;
            ig_tm_md.qid = 0x7;
	    } 
        else if(md.ow_md.type_ == ETHERTYPE_SEED) {
            // Multicast to all port?
...
control Egress(
    ...
    apply {
        ...
        if(md.ow_md.evict_ == 1) {
            ...
            md.ow_md.thd_ = (bit<8>)(md.ow_md.index_ >> 8);
            if(md.ow_md.thd_ >= 3){
                md.ow_md.index_ = md.ow_md.index_ - 1;
                hdr.ethernet.ether_type = ETHERTYPE_PUSH;
                hdr.batches.setValid();
                table_buf_pop_srcip_.apply();
                table_buf_pop_dstip_.apply();
            ...
            else if(md.ow_md.thd_ == 0){
                // If the port is the pull port?
