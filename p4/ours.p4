...
#define COLLECTOR_PORT 164
#define RECIRCULATE_PORT 132
...
control Ingress(
    ...
    action ai_forward_user(bit<9> egress_port){
        md.ow_md.entry_.portId = (bit<32>)egress_port;
    ...
    apply {
        if(hdr.ethernet.ether_type == ETHERTYPE_IPV4) {
            ...
            ig_tm_md.ucast_egress_port = COLLECTOR_PORT;
	        ig_tm_md.qid = 0x7;
	    }
    }
}
...
control Egress(
    ...
    action action_util_add_() {
        md.ow_md.entry_.utils = action_reg_util_add_.execute(md.ow_md.portId_);
    }
    ...
    apply {
        ...
        md.ow_md.timeReport_ = (md.ow_md.entry_.timeNs >> 14);
        ...
        if(md.ow_md.evict_ == 1) {
            ...
            if(md.ow_md.index_ != 65535){
                //TODO 12bit compare
                //if(hdr.buffer.size <= md.ow_md.index_){
                ...
                hdr.buffer.size = md.ow_md.index_ + 1;
            }
            //else{
