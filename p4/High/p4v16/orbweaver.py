...
def config():
    ...
    for i in range(SPINE_NUM):
        m.set_egress_mirror(sessionId=SWITCH_ID+i, mirrorPort=push_ports[i], maxLen=80)
    ...
    for i in range(len(multicast_ports)):
        ...
        m.add_rule_exact_read_action_arg(
                ...
                match_arg=multicast_ports[i],
                action_arg_name="egress_port", 
                action_arg=push_ports[i%SPINE_NUM])
...
def debug(log_time, log_dir):
    ...
    while True:
        ...
        for switchId in range(2):
            ...
            tele = m.read_reg_element_for_pipe("reg_tele_num_", switchId, pipeid=1)
            if tele != pretele[switchId]:
                print(str(switchId) + " Tele: " + str(tele))
                pretele[switchId] = tele
