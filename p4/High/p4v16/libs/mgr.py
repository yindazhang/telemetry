...
class Manager(object):
  ...
  def enab_ports_sym_bw_b(self, enab_ports, bw):
    print("In enab_ports_sym_bw_b")
    bwconfig = self.bw_dict[bw]
    print("Enable ports: "+str(enab_ports)+" with speed "+bw)
    try:
      for port in enab_ports:
  ...
  def set_ingress_mirror(self, sessionId, mirrorPort, maxLen):
    mirror_cfg_table = self.bfrt_info.table_get("$mirror.cfg")
    mirror_cfg_table.entry_add(
        self.target,
        [mirror_cfg_table.make_key([gc.KeyTuple('$sid', sessionId)])],
        [mirror_cfg_table.make_data([gc.DataTuple('$direction', str_val="INGRESS"),
                      gc.DataTuple('$ucast_egress_port', mirrorPort),
                      gc.DataTuple('$ucast_egress_port_valid', bool_val=True),
                      gc.DataTuple('$session_enable', bool_val=True)]
                      ,'$normal'
                      )])

  def set_egress_mirror(self, sessionId, mirrorPort, maxLen):
    mirror_cfg_table = self.bfrt_info.table_get("$mirror.cfg")
    mirror_cfg_table.entry_add(
        self.target,
        [mirror_cfg_table.make_key([gc.KeyTuple('$sid', sessionId)])],
        [mirror_cfg_table.make_data([gc.DataTuple('$direction', str_val="EGRESS"),
                      ...
                      gc.DataTuple('$ucast_egress_port_valid', bool_val=True),
                      gc.DataTuple('$session_enable', bool_val=True),
                      gc.DataTuple('$max_pkt_len', maxLen)]
                      ,'$normal'
                      )])
